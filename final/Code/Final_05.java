...
public class Final_05 {
	...
	private static class ListNode<T> {
		...
		long ticket; // Global sequence for full stability

		ListNode(T val, LnList<T> node, int srcIdx, long tkt) {
			value = val;
			listNode = node;
			sourceIndex = srcIdx;
			ticket = tkt;
	...
	// Global ticket counter for stable merge
	private static long globalTicket = 0;
	...
	public static <T> LnList<T> LnList_n$way$merge(LnList<T>[] xss, ToIntBiFunction<T, T> cmp) {
		...
		// Break ties by ticket (insertion order)
		java.util.Comparator<ListNode<T>> nodeComparator = (n1, n2) -> {
			int cmpResult = cmp.applyAsInt(n1.value, n2.value);
			if (cmpResult != 0) {
				return cmpResult;
			}
			// Tie: use ticket to maintain global stability
			return Long.compare(n1.ticket, n2.ticket);
		};
		...
		for (int i = 0; i < xss.length; i++) {
			if (xss[i] != null && xss[i].consq1()) {
				T headVal = xss[i].hd1();
				pq.enque$exn(new ListNode<>(headVal, xss[i], i, globalTicket++));
			}
		}
		...
		if (nextFromFirst != null && nextFromFirst.consq1()) {
			...
			pq.enque$exn(new ListNode<>(nextVal, nextFromFirst, firstSrc, globalTicket++));
		}
		...
		while (!pq.isEmpty()) {
			...
			if (nextInOriginalList != null && nextInOriginalList.consq1()) {
				...
				pq.enque$exn(new ListNode<>(nextVal, nextInOriginalList, srcIdx, globalTicket++));
			}
		}

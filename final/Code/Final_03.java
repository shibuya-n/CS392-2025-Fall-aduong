...
public class Final_03 {
	...
	static FnList<FnTupl2<FnList<Character>, Integer>> pg2701_word$count$listize3() {
		...
		// Collect all entries from the hash map
		wordMap.foritm((key, count) -> {
			// Convert string key back to FnList<Character>
			FnList<Character> word = stringToWord(key);
			// Only add the pair once per unique key (take the latest count)
		});
